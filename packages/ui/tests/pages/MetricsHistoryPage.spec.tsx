...
it('shows a loading state, then the chart region and summary totals once both metrics resolve', async () => {
  ...
  // Two calls: one for 'completed', one for 'failed', both global (no `queue`).
  expect(getHistoryMetrics).toHaveBeenCalledTimes(2);
  for (const call of getHistoryMetrics.mock.calls) {
    const params = call[0];
    expect(params.queue).toBeUndefined();
...
it('shows the empty state when both metrics resolve to no points', async () => {
  const getHistoryMetrics = jest.fn(() => Promise.resolve({ points: [] }));
...
it('renders a per-queue breakdown table sorted by completed count, querying each queue by name', async () => {
  ...
  const getHistoryMetrics = jest.fn((params: { queue?: string; metric: string }) => {
    if (!params.queue) {
      return Promise.resolve<GetMetricsHistoryResponse>({ points: [] });
    }
    if (params.queue === 'Q1') {
      return Promise.resolve<GetMetricsHistoryResponse>({
        points:
          params.metric === 'completed'
            ? [
                { ts: 1000, value: 4 },
                { ts: 2000, value: 6 },
              ]
    ...
    if (params.queue === 'Q2') {
      return Promise.resolve<GetMetricsHistoryResponse>({
        points: params.metric === 'completed' ? [{ ts: 1000, value: 3 }] : [{ ts: 1000, value: 1 }],
      });
    }
    return Promise.resolve<GetMetricsHistoryResponse>({ points: [] });
  });
  ...
  expect(perQueueCalls.length).toBe(4); // 2 metrics x 2 queues
  for (const [params] of perQueueCalls) {
    expect(['Q1', 'Q2']).toContain(params.queue);
    expect(params.granularity).toBe('day');
  }
  expect(perQueueCalls.filter(([p]) => p.queue === 'Q1').length).toBe(2);
  expect(perQueueCalls.filter(([p]) => p.queue === 'Q2').length).toBe(2);
});
