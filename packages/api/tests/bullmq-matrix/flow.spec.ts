...
describe(`Job flow on bullmq@${EXPECTED_MAJOR}`, () => {
  ...
  it('serves repeated requests from one cached producer', async () => {
    ...
    // The producer is cached in a WeakMap keyed by the raw client. A client that resolved to
    // null or undefined would throw "Invalid value used as weak map key" on the way in, which
    // is exactly how this broke under v6.
